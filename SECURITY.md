...
> **About this repository.** This is a fork of [danielmiessler/LifeOS](https://github.com/danielmiessler/LifeOS), tracking upstream releases with local modifications. The security guidance below applies to both; the reporting routes differ, see next section.

## Reporting a Vulnerability

**Please report security issues privately — do not open a public issue for anything exploitable.**

- **A vulnerability in upstream LifeOS** (anything that reproduces on a stock install) belongs upstream: use GitHub's [private vulnerability reporting](https://github.com/danielmiessler/LifeOS/security/advisories/new) on the upstream repo — the **Report a vulnerability** button under its **Security** tab. Fixing it there fixes it for every install, including this one.
- **A vulnerability specific to this fork** (present here but not upstream) belongs on this repository's own **Security** tab, via its private vulnerability reporting.
- If you are not sure which it is, report it upstream — that is the safer default.
...
- **Release-time containment gates.** Every upstream public release is built by cloning the private tree, deleting known private zones, overlaying public templates, and running a battery of gates (identity/token/secret scans, private-path leak checks, offensive-security-content checks). A single gate failure blocks the publish. "Looks clean" is never enough; the gates have to pass.

  **This applies to upstream's release pipeline, not to this fork.** This fork syncs published upstream code; it does not run that pipeline, so no gate here will catch a private path, credential, or identity string that a contribution introduces. Treat containment as manually enforced in this repository and review your own diffs accordingly.
