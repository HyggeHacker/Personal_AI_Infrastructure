...
async function cmdLaunch(options: { mcp?: string; resume?: boolean; skipPerms?: boolean; local?: boolean; systemPrompt?: string; extraArgs?: string[] }) {
  ...
  // Unknown flags passthrough — forward anything lifeos didn't recognize to claude.
  // Enables lifeos to compose with Claude Code's native flags without being aware of them.
...
async function main() {
  ...
  // Lets users combine lifeos with native Claude Code flags (--plugin-dir, --mcp-config,
  // --append-system-prompt-file, etc.) without lifeos having to know about them.
