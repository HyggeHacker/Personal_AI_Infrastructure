...
# Content precomputed by LIFEOS/TOOLS/Doctor.ts into a sidecar; %s (not %b) on
# the content so a tampered sidecar can't inject terminal escapes.
...
if _row_on use && [ "${usage_state:-absent}" = "api" ]; then
    ...
    sep
elif _row_on use && [ "${usage_state:-absent}" != "absent" ]; then
