...
A `version` source additionally requires `current_version`, the value last seen. Two optional fields carry the failure-mode handling below: `search_fallback: true` and, on a `posts` source, a `filter` string. Run `bun LIFEOS/TOOLS/Currency.ts validate <path>` to check a registry against this schema; the real `_AI_ASSESSMENT/Data/sources.json` validates clean and is the worked reference.
